...
class PPO(A2C):
    ...
    def forward_given_actions(self, state: torch.Tensor, action: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """
            ...
            Returns:
                Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]: log probability of the sample action,
                value of the given state, and entropy of the action distribution for the given state and the action tensors
        """
